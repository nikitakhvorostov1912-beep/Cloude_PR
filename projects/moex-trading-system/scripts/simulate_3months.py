...
CAPITAL = 1_000_000.0
SIM_START, SIM_END = "2025-12-18", "2026-03-18"
...
def main():
    ...
    try:
        ...
        p = generate_html_report(equity_curve, output_path="data/simulation_3months.html", title="MOEX 3-Month Simulation (Dec-Mar 2026)")
        if p: print(f"  Report: {p}")
    except Exception as e: print(f"  Report error: {e}")
    Path("data/simulation_3months.json").write_text(json.dumps({"return": round(ret*100,2), "trades": len(cl), "pf": round(pf,2), "trade_log": trades}, indent=2, ensure_ascii=False))
    print(f"  JSON: data/simulation_3months.json")
