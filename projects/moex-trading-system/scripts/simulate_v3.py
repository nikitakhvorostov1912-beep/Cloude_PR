"""V3 Full Simulation — ALL modules, ALL instruments, ALL strategies.

Uses EVERYTHING built in the system:
- 249 stocks + Si/BR futures + IMOEX index
- ML ensemble (honest, train before sim)
- Real macro data (USDRUB, Brent from DB, IMOEX vs SMA200)
- News reactor (599 articles, classify impact)
- Kelly Criterion for position sizing
- ATR-based stops + trailing stops + time stops
- Sector rotation via macro alignment
- Universe ranking: ML(40%) + Momentum(25%) + Macro(20%) + RS(15%)
- Futures: Si hedge when portfolio exposed, BR for oil plays

Usage:
    python scripts/simulate_v3.py
"""
...
def main():
    ...
    for today in sim_dates:
        ...
        if n_stock < MAX_POS and not critical:
            ...
            for tk, dfeats in tk_feats.items():
                ...
                ml = max(0, min(100, ml + news_adj.get(tk, 0)))
    ...
    try:
        ...
        p = generate_html_report(equity_curve, output_path="data/simulation_v3.html", title="MOEX V3 Full Simulation")
        if p: print(f"  Report: {p}")
    except Exception as e: print(f"  Report error: {e}")
    Path("data/simulation_v3.json").write_text(json.dumps({"return": round(ret*100,2), "trades": len(cl), "pf": round(pf,2), "trade_log": trades}, indent=2, ensure_ascii=False))
    print(f"  JSON: data/simulation_v3.json")
