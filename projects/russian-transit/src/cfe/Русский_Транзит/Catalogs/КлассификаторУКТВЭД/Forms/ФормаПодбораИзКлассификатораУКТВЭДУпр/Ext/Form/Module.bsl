...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//начало крамол окс 08.07.2021 19:44:41 
	Макет = Справочники.КлассификаторУКТВЭД.ПолучитьМакет("КлассификаторУКТВЭД_Упр");
	//конец крамол окс 08.07.2021 19:44:44 
	
	//Макет = НормативносправочныйИнформационныйСервисСервер.ПолучитьМакет("КлассификаторУКТВЭД", Справочники.КлассификаторУКТВЭД.ПолучитьМакет("КлассификаторУКТВЭД_Упр"));
...
//начало крамол окс 08.07.2021 20:27:31 
&НаКлиенте
Процедура ОтметитьВсе(Команда)
	
	СтрокиДерева = ДеревоКлассификатора.ПолучитьЭлементы();
	Для каждого СтрД Из СтрокиДерева Цикл
		СтрД.Выбран = 1;
		ОбходДереваВниз(СтрД);
	КонецЦикла;
...
КонецПроцедуры
//конец крамол окс 08.07.2021 20:27:34  
